...
duplicate_rows <- function(data, meta) {
  ...
  if (nrow(data %>% distinct(geographic_level)) == 1 &
    ...
    if (nrow(dupes) > 0) {
      output <- list(
        "message" = paste("There are", cs_num(nrow(dupes)), "duplicate rows in the data file. <br> - Note that", paste0(geography_matrix[14, 1], ", ", geography_matrix[15, 1], ", ", geography_matrix[15, 1], " and ", geography_matrix[16, 1]), "level rows were not included in this test."),
        "result" = "FAIL"
      )
    } else {
      output <- list(
